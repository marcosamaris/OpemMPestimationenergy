...
float pspeedy(Points *points, float z, long *kcenter, int pid, pthread_barrier_t* barrier)
{
...
#ifdef PRINTINFO
  if( pid == 0 ){
    //(stderr, "Speedy: facility cost %lf\n", z);
  }
#endif
  ...
  if( pid == 0 )
    {
      //(stderr, "Speedy opened %d facilities for total cost %lf\n",
	      *kcenter, totalcost);
      //(stderr, "Distance Cost %lf\n", totalcost - z*(*kcenter));
    }
#endif
...
#ifdef PRINTINFO
    if( pid == 0 ) {
      //(stderr, "%d centers, cost %lf, total distance %lf\n",
	      *k, cost, cost - z*(*k));
    }
#endif
#ifdef ENABLE_THREADS
		    ...
		    long chunksize, long centersize, char* outfile )
{
  ...
  if( block == NULL ) { 
    //(stderr,"not enough memory for a chunk!\n");
    exit(1);
  }
  ...
  while(1) {
    ...
    //(stderr,"read %d points\n",numRead);

    if( stream->ferror() || numRead < (unsigned int)chunksize && !stream->feof() ) {
      //(stderr, "error reading data!\n");
      exit(1);
    }
    ...
    //(stderr,"finish local search\n");
    contcenters(&points);
    if( kfinal + centers.num > centersize ) {
      //here we don't handle the situation where # of centers gets too large. 
      //(stderr,"oops! no more space for centers\n");
      exit(1);
    }
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  if (argc<10) {
    //(stderr,"usage: %s k1 k2 d n chunksize clustersize infile outfile nproc\n",
	  //  argv[0]);
    //(stderr,"  k1:          Min. number of centers allowed\n");
    //(stderr,"  k2:          Max. number of centers allowed\n");
    //(stderr,"  d:           Dimension of each data point\n");
    //(stderr,"  n:           Number of data points\n");
    //(stderr,"  chunksize:   Number of data points to handle per step\n");
    //(stderr,"  clustersize: Maximum number of intermediate centers\n");
    //(stderr,"  infile:      Input file (if n<=0)\n");
    //(stderr,"  outfile:     Output file\n");
    //(stderr,"  nproc:       Number of threads to use\n");
    //(stderr,"\n");
    //(stderr, "if n > 0, points will be randomly generated instead of reading from infile.\n");
    exit(1);
  }
  kmin = atoi(argv[1]);
  kmax = atoi(argv[2]);
